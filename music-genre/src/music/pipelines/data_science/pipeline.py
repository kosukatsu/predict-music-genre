...
def create_cross_validation_pipeline(**kwargs):
    return Pipeline(
        [
            node(drop_feature,["train_data_set","params:drop_feature"],"train_data_dropped_feature"),
            node(merge_pseudo_data,["train_data_dropped_feature","pseudo_data","params:use_pseudo_label"],"train_data_concated_pseudo"),
...
def create_real_train_pipeline(**kwargs):
    return Pipeline(
        [
            node(merge_dictionary,["params:default_lgbm_params","params:lgbm_params"],"lgbm_params"),
            node(drop_feature,["train_data_set","params:drop_feature"],"train_data_dropped_feature"),
            node(merge_pseudo_data,["train_data_dropped_feature","pseudo_data","params:use_pseudo_label"],"train_data_concated_pseudo"),
            node(split_data, "train_data_concated_pseudo", ["train_x", "train_y"],),
            node(
...
def create_eval_pipeline(**kwargs):
    return Pipeline([
        node(drop_feature,["test_data_set","params:drop_feature"],"test_data_dropped_feature"),
        node(predict, ["test_data_dropped_feature", "lgbm_model"], "lgbm_output"),])

def create_pseudo_label_pipeline(**kwargs):
    return Pipeline([
        node(drop_feature,["test_data_set","params:drop_feature"],"test_data_dropped_feature"),
        node(pseudo_label, ["test_data_dropped_feature", "lgbm_model","params:importance_threthold"], "pseudo_data"),
        ]
        )
